                ...
                {columns.map((column) => (
                  <TableCell key={`${index}-${String(column.key)}`} className="whitespace-nowrap">
                    { column.key === 'referrer_mobile' || column.key === 'under_admin_mail' ? String(row[column.key]) === 'null' ? '' : String(row[column.key])
                     ...
                     : column.key === 'amount' || column.key === 'balance' || column.key === 'wins' || column.key === 'commissions' || column.key === 'maintaining_balance' || column.key === 'quota' || column.key === 'bet'? 
...
export function MetricDetailView() {
  ...
  useEffect(() => {
    ...
    }, [startDate, endDate, playerStatus, metricId]);

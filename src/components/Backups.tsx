...
import { loginAdmin, getBetsHistory, getLogs, getAuditLogs, getBackups, deleteBackup } from "./api/apiCalls";
...
export function Backups() {
      ...
      const handleUpdate = async () => {
        try {
          ...
          if (dataUpdated.dbUpdate) {
            ...
            const gamesData = await getBackups();
  ...
  const handleDownloadClick = (game) => {
    ...
    link.href = API_URL + `/admin/downloadBackup?id=${game.id}`;
    link.setAttribute('download', ''); // Just hint
  ...
  const deleteFileClick = async (game) => {
    await deleteBackup(game.id);
              ...
              <TableBody>
                {paginatedBets.map((product) => (
                  <TableRow
                    key={product.id}
                  >
                     <TableCell className="text-center">{product.date_time}</TableCell>
                     <TableCell className="text-center">{product.log_type}</TableCell>
                    <TableCell className="text-center">{product.file}</TableCell>
                     <TableCell className="text-center">
                      <Button
                        className="w-full sm:w-auto bg-red-500 border-red-500 text-black-600 hover:bg-red-500/20 hover:text-red-700"
        ...
        {isModalOpen && (
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
            <DialogContent className="bg-gray-50 border-[#34495e] max-h-[90vh] w-96 overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="text-xl text-blue-600">Are you sure you want to delete?</DialogTitle>
              </DialogHeader>
              <div className="space-y-3">
                  ...
                  <DialogFooter className="flex flex-col gap-2 sm:flex-row">
                      ...
                      <Button
                        variant="outline"
                        className="w-full sm:w-auto bg-red-500 border-red-500 text-black-600 hover:bg-red-500/20 hover:text-red-700"
                        onClick={() => deleteFileClick(selectedGameBet)}
                      >
                        Confirm Delete
                      </Button>
                    <Button

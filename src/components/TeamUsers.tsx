...
import { loginAdmin, getPlayers, updatePlayer, getPlayersAdminChoice, updatePlayerTeam, getPlayersTeam } from './api/apiCalls';
...
export function TeamUsers() {
    ...
    if (user && !dbUpdated) {
      const handleUpdate = async () => {
        ...
        if (dataUpdated.dbUpdate) {
          ...
          setLoading(false);
          const gamesData = await getPlayersTeam(dataUpdated.userID);
  ...
  if(!permissionsString.includes("team_players"))
  {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!isAuthenticated) {
      ...
      const gamesData = await getPlayersTeam(userID);
      setGamebets(gamesData);
      setFilteredGamebets(gamesData);
    }
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl md:text-3xl font-bold">List of My Team</h2>
      </div>
